General:
    ...
    Resolved an issue that would sometimes cause the proper FX to not play when using Feign Disintegration power.

    Resolved an issue where the playing cards in the player's hand could occasionally persist after the emote has ended.
...
Ships:
    ...
    Adjusted the firing nodes on the Kelvin Constitution Beyond Refit so that phasers in the 5th node fire from the correct spot on the hull.

    The Columbia Class Light Escort Nacelles are now correctly paired when selected in the ship customization screen.
...
Systems:
    ...
    Resolved an issue that caused most Legendary Ships to display an incorrect description of their Starship Traits.

    Resolved an issue that prevented Saucer Separation from separating your saucer when activated on the Legendary Command Exploration Cruiser [T6]

    Resolved an issue that caused the Legendary Command Dreadnought Cruiser to be missing the Odyssey bridges.

    Resolved an issue that caused the Legendary Pilot Warship to have the Escort Mastery package, instead of the Warship Mastery package.

    Resolved an issue that caused the Legendary Miracle Worker Multi-Mission Science Vessel to not animate correctly in some cases.

    Resolved an issue that caused the Legendary Miracle Worker Assault Cruiser [T6] to not come equipped with the Wide-Angle Quantum Torpedo Launcher.

    Resolved an issue that caused the Legendary Kelvin Timeline Intel Battlecruiser to be missing a 5th fore weapon in some cases
